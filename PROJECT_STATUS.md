...
- [~] Phase 3: Learning progress (real XP, levels, and streak tracking implemented and verified live: XP is derived from completed lessons (20 XP each, no fabricated numbers), levels are derived from total XP (100 XP per level), streaks count consecutive calendar days with at least one lesson completed, tracked in a real Streak table and updated by the mark-complete server action. A first real achievements system is also implemented - five achievements ("First Steps", "Getting Serious", "Streak Starter", "Streak Keeper", "World Graduate") computed entirely from real lesson/streak/world-completion data, shown as Earned/Locked on the dashboard. Mastery scoring is now implemented (masteryLevelFromScore maps each skill's score to NOT_STARTED/INTRODUCED/PRACTICING/DEVELOPING/PROFICIENT/MASTERED) and missions are now real, server-graded challenges - see Phase 4)
- [x] Phase 4: Interactive missions (real server-graded submissions for all mission types - multiple_choice, predict_output/text_exact, debug_challenge/regex_all or nesting_order, code_writing/regex_all, and js_run via a sandboxed Node vm - verified live: solving a mission awards XP once on first pass, updates the related skill's mastery score, and is reflected immediately on the dashboard)
...
- [~] Phase 6: Launch curriculum (5 of 17 worlds now have real, launch-quality content - Web Foundations, HTML Harbor, CSS City, JavaScript Jungle, and TypeScript Tower - each with 2 modules, 4 lessons, and 4 missions, 20 lessons and 20 missions total, all missions server-graded. Still below CLAUDE.md's minimums of 30 lessons and 100 missions; the other 12 worlds remain correctly marked "Upcoming")
...
Completed Phase 4 (interactive mission grading) end to end: prisma/seed.ts now carries a prompt/options/grading-spec (MissionTest.expected) for every mission; src/lib/mission-grading.ts dispatches by checkType (mc, text_exact, regex_all, nesting_order, js_run via a sandboxed Node vm); the submitMissionAttempt server action re-validates everything server-side and awards XP/updates skill mastery only on a learner's first correct pass; and the mission-solver UI replaced the old "Coming soon" placeholder with a real form per mission type. Verified live in production: multiple_choice and predict_output missions were solved end to end (correct XP awarded, dashboard updated). Also added a fifth real, published world - TypeScript Tower (2 modules, 4 lessons, 4 missions covering type annotations, inference, typed functions, and interfaces) - bringing the total to 5 published worlds, 20 lessons, and 20 gradeable missions. One self-inflicted bug from this work is worth recording: a clipboard paste meant to insert new mission grading specs landed one line too early inside an existing object literal, which was not caught until Railway's build failed on a TypeScript error; it was diagnosed from the build logs and fixed with two precise brace corrections, verified by re-parsing the file's brace balance before committing again. Next logical step: keep expanding real curriculum content toward CLAUDE.md's launch minimums (30 lessons, 100 missions), and/or build notes/review features. Proceeding autonomously per user direction to keep building real, non-placeholder features.
...
- Curriculum engine: real content now spans 5 of 17 worlds (10 modules / 20 lessons / 20 missions total), with real visual/gamification polish; CLAUDE.md section 12 requires at least 30 lessons and 100 missions at launch - more content is still needed
- Learning progress: real XP, level, streak, and a first achievements system are now implemented and verified live across five worlds; mastery scoring is implemented (masteryLevelFromScore)
- Interactive code editor and safe execution, mission submission/grading - implemented and verified live (plain textarea code input, not a full Monaco/CodeMirror editor - see KNOWN_ISSUES.md)
